...
will examine both the male and female average compound lifts (bench, squat, and deadlift)
  ...
  index <- data |>
            filter(!is.na(age)) |> 
              filter(!is.na(age_class)) |>
                group_by(age_class) |>
                  filter((total_kg == max(total_kg, na.rm = TRUE))) |>
                    select(name, age, age_class, total_kg) |>
                        distinct(name, age, age_class, total_kg) |> 
                          arrange(age_class)
