...
deteriorates over time. We set out to see which age class is the strongest on average
in hopes to discover when the body is at its peak strength while consistently working
out. It is expected for the peak age class to be 24-34, where the body is building
strength before this mark but losing strength after this mark. This will be explored
in both men and women to see if they a follow similar strength pattern.


first start by splitting data up by the corresponding age groups
then calculate their average totals
then compare and see if there are any noticable trends
go a step further and see which ages within the age classes are the strongest

# Functions for Strength by Age
```{r}

get_age_averages <- function(data){
  result <- data |> 
              avg_weights(data)
  return (result)
}
...
age_levels = c("5-12", "13-15", "16-17", "18-19", "20-23", "24-34", "35-39", "40-44", "45-49", "50-54", "55-59", "60-64", "65-69", "70-74", "75-79", "80-999")

# get only the men, and turn the age_class into a factor for organization
men_averages <- ipf |> filter(sex == "M") |>
                mutate(age_class = factor(age_class, levels = mens_levels))

# Now we can perform our actual calculations for men
result <- avg_weights(men_averages, age_class)
result
```
