...
rates
# graph the rates, success and fail for each lift. bar out of 100 with the percentage making
# the 100
rates |> ggplot(aes(x = lift, y = failed, fill = lift)) + 
    geom_bar(stat = "identity", position = position_stack()) +
    labs(
        title = "Percentage of Failed Lifts for Each Lift",
        x = "Lift",
        y = "Percentage (%)"
    )
...
lift_data_long <- rates |>
  gather(key = "outcome", value = "percentage", -lift); lift_data_long
...
ggplot(lift_data_long, aes(x = lift, y = percentage, fill = outcome)) +
    ...
    scale_fill_manual(values = c("failed" = "red", "success" = "green"))
```
